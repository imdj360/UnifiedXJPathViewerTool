...
exports.normalizeJSONPath = normalizeJSONPath;
exports.ensureDefaultXPathIndexes = ensureDefaultXPathIndexes;
/**
 ...
 */
function toXPath(node, compactPaths = false) {
    const segments = [];
    let current = node;
 ...
 */
function toJSONPath(segments, compactPaths = false) {
    if (!segments.length) {
        return '$';
    }
    const parts = ['$'];
...
function parseJSONPath(path) {
    ...
    while (index < path.length) {
        ...
        if (char === '.') {
            ...
            if (path[index] === '\'' || path[index] === '"') {
                ...
                while (index < path.length) {
                    ...
                    if (current === '\\') {
                        ...
                        index++;
                        if (path[index] !== ']') {
                            throw new Error(`Expected ] after quoted segment at position ${index}`);
                        }
                        index++;
                        segments.push(value);
                        break;
            ...
            throw new Error(`Unexpected character "${char}" in JSONPath at position ${index}`);
        }
    }
    return segments;
}
 ...
 */
function normalizeJSONPath(path, compactPaths = false) {
...
const ELEMENT_STEP_REGEX = new RegExp(`^${SIMPLE_NAME_WITH_PREFIX}(?:\\[\\d+\\])?$`);
 ...
 */
function ensureDefaultXPathIndexes(expression) {
